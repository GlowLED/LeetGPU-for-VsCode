...
- `https://yhdtysacdkqoquvkdwdd.supabase.co` for imported-session exchange and session refresh.
...
The resulting session is stored using VS Code `SecretStorage`. Session values are never written to workspace files or extension logs. Direct GitHub and Google browser callbacks are currently disabled.
